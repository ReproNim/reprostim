...
/**
 * @ingroup group_variables_enum
 * @brief MWCAP_SDI_SINGLE_LINK_STATUS
 * @details Get connection status of Dual/Quad link SDI signal \n
 */
typedef struct _MWCAP_SDI_SINGLE_LINK_STATUS {
	BYTE										byPortIndex;	///<Input parameter, index of SDI port
	BOOLEAN										bLinked;		///<Output parameter, whether the spcified port has input signal.
	MWCAP_SDI_SPECIFIC_STATUS					sdiStatus;		///<Output parameter, if the "bLinked" is true, "sdiStatus" save detailed information of the signal.
} MWCAP_SDI_SINGLE_LINK_STATUS;
...
/**
 * @ingroup group_variables_macro
 * @brief MWCAP_NOTIFY_SDI_LINK_STATUS_CHANGED
 * @details Event notification. Connection status of multiple link SDI signal changed.\n
 * Related functions are:\n
 * 			[MWRegisterNotify](@ref MWRegisterNotify)\n
 * 			[MWUnregisterNotify](@ref MWUnregisterNotify)\n
*/
#define MWCAP_NOTIFY_SDI_LINK_STATUS_CHANGED		0x40000ULL
 ...
 *                         [MWUnregisterNotify](@ref MWUnregisterNotify)\n
*/
#define MWCAP_NOTIFY_HDMI_INFOFRAME_VBI                                (1ULL << (32 + MWCAP_HDMI_INFOFRAME_ID_VBI))
 ...
 *                         [MWUnregisterNotify](@ref MWUnregisterNotify)\n
*/
#define MWCAP_NOTIFY_HDMI_INFOFRAME_HDR                                (1ULL << (32 + MWCAP_HDMI_INFOFRAME_ID_HDR))
...
/**
 * @ingroup group_variables_struct
 * @brief MWCAP_AUDIO_CAPTURE_FRAME_16CH
 * @details Records audio frame info.\n
 * Related functions are:\n
 * 		   [MWCaptureAudioFrame16Ch](@ref MWCaptureAudioFrame16Ch)\n
*/
typedef struct _MWCAP_AUDIO_CAPTURE_FRAME_16CH {
    DWORD										cFrameCount;
    DWORD										iFrame;
    DWORD										dwSyncCode;
    DWORD										dwFlags;
    ...
    BYTE										cBitsPerSample;
    DWORD										dwSampleRate;
    DWORD										adwSamples[MWCAP_AUDIO_SAMPLES_PER_FRAME * 16];
} MWCAP_AUDIO_CAPTURE_FRAME_16CH;
...
/**
 * @ingroup group_variables_macro
 * @brief MWCAP_FRAME_BUF_TYPE_USER
 * @details  user buffer type \n
*/
#define MWCAP_FRAME_BUF_TYPE_USER       (1)


/*
 * @ingroup group_variables_macro
 * @brief MWCAP_FRAME_BUF_TYPE_DMABUF
 * @details  dma buffer type\n
*/
#define MWCAP_FRAME_BUF_TYPE_DMABUF     (2)

/**
 * @ingroup group_variables_struct
 * @brief MWCAP_VIDEO_ECO_OCOPY_CAPTURE_OPEN
 * @details ECO zero capture configurations,MAC doesn't support\n
*/
typedef struct _MWCAP_VIDEO_ECO_OCOPY_CAPTURE_OPEN {
    MWCAP_PTR                                       hEvent;				///<Handle of capture event

    DWORD                                           dwFOURCC;			///<Capture format
    WORD                                            cx;					///<Width
    WORD                                            cy;					///<Height
    LONGLONG                                        llFrameDuration;    ///<Interval, -1 indicates follow format of input source
} MWCAP_VIDEO_ECO_OCOPY_CAPTURE_OPEN;

/**
 * @ingroup group_variables_struct
 * @brief MWCAP_VIDEO_ECO_CAPTURE_FRAME
 * @details ECO zero capture frames,MAC doesn't support\n
*/
typedef struct _MWCAP_VIDEO_ECO_OCOPY_CAPTURE_FRAME {
    BYTE                                            bufType;		///<Type of the frame buffer. Valid options: [MWCAP_FRAME_BUF_TYPE_USER](@ref MWCAP_FRAME_BUF_TYPE_USER ), [MWCAP_FRAME_BUF_TYPE_DMABUF](@ref MWCAP_FRAME_BUF_TYPE_DMABUF )		
    ULONG                                           bufHandle;		///<Handle of the frame buffer
    MWCAP_PTR                                       pvContext;		///<Frame label
    
    DWORD                                           cbFrame;		///<Size of video frame
    DWORD                                           cbStride;  		///<Stride of video frame
    DWORD                                           bufUUID; 		///<UUID of the buffer

    LONGLONG                                        llTimestamp;	///<Timestamp of video frame
    BOOLEAN                                         bFrameError; 	///<Indicate the frame status
} MWCAP_VIDEO_ECO_OCOPY_CAPTURE_FRAME;


/**
 * @ingroup group_variables_struct
 * @brief MWCAP_SDI_QUAD_LINK_MANUAL_MODE
 * @details Mode of SDI qual link signal\n
*/
typedef enum _MWCAP_SDI_QUAD_LINK_MANUAL_MODE {
	MWCAP_SDI_QUAD_LINK_MANUAL_MODE_OFF = 0,	///<Detect the mode automatically
	MWCAP_SDI_QUAD_LINK_MANUAL_2SI,				///<Fix the mode to 2si			
    MWCAP_SDI_QUAD_LINK_MANUAL_SQD,				///<Fix the mode to sqd	
} MWCAP_SDI_QUAD_LINK_MANUAL_MODE;
