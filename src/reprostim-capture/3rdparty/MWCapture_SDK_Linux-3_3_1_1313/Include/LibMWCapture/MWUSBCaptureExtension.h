...
typedef struct _MWCAP_USB_FIRMWARE_DATA {
	unsigned int        offset;
    unsigned int        size; /* data size */
    MWCAP_PTR           data[256]; /* (unsigned char * data) */
} MWCAP_USB_FIRMWARE_DATA;
...
typedef struct _MWCAP_VIDEO_FRAMERATE{
	int				nFrameDuration;
	float			fFPS;
}MWCAP_VIDEO_FRAMERATE;


typedef struct _MWCAP_VIDEO_FRAMERATE_EX{
	int				nFrameDurationDen;
	int				nFrameDurationNum;
