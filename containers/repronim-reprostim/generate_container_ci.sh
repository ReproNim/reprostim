...
echo "REPROSTIM_GIT_HOME: ${REPROSTIM_GIT_HOME}"
echo "REPROSTIM_HOME: ${REPROSTIM_HOME}"
...
generate() {
	...
	docker run --rm repronim/neurodocker:$ndversion generate "$1" \
        ...
        --copy "${REPROSTIM_GIT_HOME}" "${REPROSTIM_HOME}" \
        --run "${PSYCHOPY_HOME}/bin/pip install ${REPROSTIM_HOME}[all,disp_mon]" \
