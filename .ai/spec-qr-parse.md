...
| `-g / --grayscale [none\|mean\|cvtcolor]`  | str   | `cvtcolor` | Grayscale conversion method applied to each frame before QR decoding. `cvtcolor` uses `cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)` (fastest, recommended). `mean` uses `np.mean(frame, axis=2)` (slow, legacy). `none` passes the raw frame directly — may cause runtime errors with decoders that expect a single-channel image.                             |
